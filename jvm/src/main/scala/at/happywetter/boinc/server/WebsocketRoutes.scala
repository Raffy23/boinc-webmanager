...
object WebsocketRoutes:
    ...
    def apply(token: String): IO[Client] =
      for {
        ...
        result <- Queue
          ...
          .map { responseQueue =>
            val callback: BoincManager => IO[Unit] = manager =>
              for {
  ...
  def apply(webSocketBuilder: WebSocketBuilder[IO],
    ...
    case GET -> Root :? JWTToken(token) if authService.validate(token).getOrElse(false) =>
      Client(token).flatMap { client =>
        val clientReply: PartialFunction[WebSocketFrame, IO[WebSocketFrame]] =
          case Binary(msg, _) =>
            readBinary[WebSocketMessage](msg.toArray) match

              case SubscribeToGroupChanges =>
                boincManager.changeListener
              ...
              client.state.supervisor
                .supervise(
                  clientReply
                    .lift(payload)
                    .map { responseIO =>
                      responseIO.flatMap(response => client.responseQueue.offer(Some(response)))
                    }
                    .getOrElse(IO.unit)
                )
                .unsafeRunAndForget()
  ...
  implicit private def convertWebSocketMessageToWebSocketFrame[T <: WebSocketMessage](msg: T)(implicit
    writer: Writer[T]
  ): WebSocketFrame =
