...
object ServerStatusParser:
  ...
  def fromXML(node: NodeSeq) = ServerStatus(
    ...
    if (node \ "database_file_states" \ "tasks_by_app").exists(_.label == "tasks_by_app") then {
  ...
  implicit private class OrDSL(val node: NodeSeq) extends AnyVal:
    def or(other: NodeSeq): String = if node.text.isEmpty then other.text else node.text
    def or(f: NodeSeq => Boolean): NodeSeq = if node.theSeq.size > 1 then node.filter(f) else node

  private val findTotal: NodeSeq => Boolean = node => node \@ "app" == "total"

  implicit private def convertNodeSeqToString(node: NodeSeq): String = node.text
  implicit private def convertNodeSeqToInteger(node: NodeSeq): Int = node.tryToInt
  implicit private def convertNodeSeqToDouble(node: NodeSeq): Double = node.tryToDouble
