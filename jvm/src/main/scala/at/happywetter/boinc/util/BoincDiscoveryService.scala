...
/**
  ...
  * @version 19.09.2017
  */
class BoincDiscoveryService private (config: AutoDiscovery,
                                     excludedHosts: Ref[IO, Set[Host]],
                                     autoScanCallback: Seq[Host] => IO[Unit],
                                     lock: Semaphore[IO],
...
object BoincDiscoveryService:
  ...
  def apply(config: AutoDiscovery,
            boincManager: BoincManager,
            autoScanCallback: Seq[Host] => IO[Unit]
  ): Resource[IO, BoincDiscoveryService] =
    for {
      service <- Resource.eval(
        for {
          ...
          _ <- boincManager.changeListener.register(mgr =>
            for {
  ...
  implicit private class IpAddressToAble(private val ip: IpAddress) extends AnyVal:
