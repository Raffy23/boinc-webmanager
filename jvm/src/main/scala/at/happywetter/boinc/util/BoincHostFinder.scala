...
/**
  ...
  * @version 20.09.2017
  */
class BoincHostFinder private (config: Config, boincManager: BoincManager, db: Database, logger: SelfAwareStructuredLogger[IO]) {
  ...
  private def discoveryCompleted(hosts: Seq[Host]): IO[Unit] = {
    getUsedHosts.flatMap { usedIPs =>
      import cats.implicits._

      (
        if (hosts.nonEmpty) logger.info("Following hosts can be added: " + hosts.diff(usedIPs))
