...
const scalaOutputPath = path.resolve(__dirname, `./js/target/scala-${scalaVersion}`);
...
const common = (mode) => ({
  ...
  entry: [
    path.resolve(__dirname, './js/src/main/static/stylesheets/main.scss'),
    path.resolve(__dirname, './js/src/main/static/stylesheets/main.css')
  ],
  module: {
    rules: [
      {
  ...
  plugins: [
    new HtmlWebpackPlugin({
      filename: 'index.html',
      template: './js/src/main/static/html/index.html.ejs',
      minify: false,
      inject: 'head',
    }),
    new CopyWebpackPlugin({
      patterns: [
        {from: './js/src/main/static/public', to: ''},
      ]
    })
  ]
})
...
const dev = {
  ...
  entry: [
    path.resolve(__dirname, `${scalaOutputPath}/boinc-webmanager_client-fastopt/main.js`),
  ],
  plugins: [
...
const prod = {
  entry: [
    path.resolve(__dirname, `${scalaOutputPath}/boinc-webmanager_client-opt/main.js`),
  ],
  // devtool: 'source-map',
  optimization: {
    minimize: true,
