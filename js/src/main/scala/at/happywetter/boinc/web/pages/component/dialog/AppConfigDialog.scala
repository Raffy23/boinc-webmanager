...
class AppConfigDialog(parentID: String, project: Project)(implicit boinc: BoincClient) extends Dialog("appconfig-dialog") {

  NProgress.start()
  private val loading = Var(0)

  private val apps = boinc.getState.map(state => {
    loading.update(_ + 1)
    state.apps.filter { case (_, app) => app.project == project.url }
  }).toRx(Map.empty)

  private val config = boinc.getAppConfig(project.url).map(r => {loading.update(_+1); r}).toRx(
    ...
    <div class={Style.modal.htmlClass} id="appconfig-dialog" onclick={jsBackgroundAction}>
      <div class={Style.content.htmlClass}  style="max-width:85%">
        ...
        <div class={Style.body.htmlClass} style={s"max-height:${maxHeight}px"}>
          ...
          <ul>
            <li>{"is_loading".localize}{": "}{loading.map(p => {if (p==2) NProgress.done(true); p != 2}).map(_.localize)}</li>
            <li>{"has_app_config".localize}{": "}{config.map(c => c.apps.nonEmpty || c.appVersions.nonEmpty).map(_.localize)}</li>
            <li>{"max_concurrent".localize}{": "}{config.map(_.projectMaxConcurrent.map(_.toString).getOrElse("not_set".localize))}</li>
          ...
          <table class={Seq(TableTheme.table.htmlClass, TableTheme.noBorder.htmlClass).mkString(" ")}>
            <thead>
              <th class={TableTheme.verticalText.htmlClass}><div style="margin-left:calc(100% - 8px);margin-bottom:1px"><span>{"user_friendly_name".localize}</span></div></th>
              <th class={TableTheme.verticalText.htmlClass}><div style="margin-left:calc(100% - 8px);margin-bottom:1px"><span>{"app_name".localize}</span></div></th>
              <th class={TableTheme.verticalText.htmlClass}><div style="margin-left:calc(100% - 8px);margin-bottom:1px"><span>{"ngpus".localize}</span></div></th>
              <th class={TableTheme.verticalText.htmlClass}><div style="margin-left:calc(100% - 8px);margin-bottom:1px"><span>{"avg_cpus".localize}</span></div></th>
              <th class={TableTheme.verticalText.htmlClass}><div style="margin-left:calc(100% - 8px);margin-bottom:1px"><span>{"plan_class".localize}</span></div></th>
              <th class={TableTheme.verticalText.htmlClass}><div style="margin-left:calc(100% - 8px);margin-bottom:1px"><span>{"cmdline".localize}</span></div></th>
              <th class={TableTheme.verticalText.htmlClass}><div style="margin-left:calc(100% - 8px);margin-bottom:1px"><span>{"report_results_immediately".localize}</span></div></th>
              <th class={TableTheme.verticalText.htmlClass}><div style="margin-left:calc(100% - 8px);margin-bottom:1px"><span>{"max_concurrent".localize}</span></div></th>
