...
import at.happywetter.boinc.shared.{App, Workunit}
...
object AppSettingsStorage extends DatabaseProvider {
  ...
  def save(boincName: String, wu: Workunit): Future[Unit] = {
    transaction.map(f => f.put(js.Dynamic.literal("appName" -> wu.appName,"client"->boincName, "wuname"->wu.name)))
  }
  ...
  def get(boincName: String, wuName: String): Future[Option[Workunit]] = {
    ...
    transaction.flatMap(f => unpack(f.get(key)))
  }
