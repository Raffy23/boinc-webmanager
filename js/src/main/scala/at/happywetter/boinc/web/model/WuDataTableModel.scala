...
import at.happywetter.boinc.web.pages.component.DataTable.LinkColumn
...
object WuDataTableModel:
  ...
  class ReactiveResult(result: Result)(implicit boinc: BoincClient):
    ...
    val app: Rx[Option[App]] =
      AppSettingsStorage
        .get(boinc.hostname, result.wuName)
        .flatMap(wu => {
  ...
  class WuTableRow(val result: ReactiveResult)(implicit boinc: BoincClient) extends DataTable.TableRow:
    ...
    override val columns = List(
      ...
      new TableColumn(
        ...
        override def compare(that: TableColumn): Int =
          result.progress.now.compare(that.datasource.asInstanceOf[WuTableRow].result.progress.now)
      },
      new StringColumn(result.uiStatus),
      new StringColumn(result.pastTime.map(_.toTime)) {
        override def compare(that: TableColumn): Int =
          result.pastTime.now.compare(that.datasource.asInstanceOf[WuTableRow].result.pastTime.now)
      },
      new StringColumn(result.remainingCPU.map(_.toTime)) {
        override def compare(that: TableColumn): Int =
          result.remainingCPU.now.compare(that.datasource.asInstanceOf[WuTableRow].result.remainingCPU.now)
      },
      new StringColumn(result.reportDeadline.map(_.toDate)) {
        override def compare(that: TableColumn): Int =
          result.reportDeadline.now.compare(that.datasource.asInstanceOf[WuTableRow].result.reportDeadline.now)
      },
    ...
    private lazy val cancelDialog = new SimpleModalDialog(
          ...
          <p>
            ...
            <table class={TableTheme.table.htmlClass}>
              <tbody>
                ...
                <tr><td>{"workunit_dialog_cancel_remaining_time".localize}</td><td>{
          result.remainingCPU.map(_.toTime)
        }</td></tr>
              </tbody>
            </table>
          </p>
      ...
      okAction = (dialog: SimpleModalDialog) => {
        dialog.hide()
        boinc
          .workunit(result.projectURI, result.name.now, WorkunitAction.Abort)
          .map(result => {
            if (!result) {
        ...
        <table class={TableTheme.table.htmlClass}>
          ...
          result.activeTask.map(_.map(task => {
            List( // mthml does not like NodeBuffer -> wrap it into a list
              ...
              <tr><td><b>{"wu_dialog_progress".localize}</b></td><td>{"%.4f %%".format(task.done * 100)}</td></tr>,
    ...
    private lazy val jsPauseAction: (Event) => Unit = event => {
      ...
      boinc
        .workunit(result.projectURI, result.name.now, action)
        .map(response => {
          if (!response) {
        ...
        result.activeTask
          .map(task =>
            Result.ActiveTaskState(task.activeTaskState) match {
              ...
              case Result.ActiveTaskState.PROCESS_SUSPENDED =>
                if (result.supsended) "boinc_status_suspend1".localize else "boinc_status_suspend2".localize
    ...
    app
      ...
      if (app.version.coproc.isDefined) {
        ...
        if (app.version.avgCpus != 1 && !app.nonCpuIntensive && result.resources.isEmpty)
          props = s"${"%.2f".format(app.version.avgCpus)} CPUs" :: props
