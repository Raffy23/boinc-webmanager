...
object WebRPCProjectPage extends Layout:
  ...
  def convertDaemonStatus(status: ServerStatus): Elem =
    <table class={TableTheme.table.htmlClass}>
      ...
      <tbody>{
      status.daemon_status.map { daemon =>
        <tr><td>{daemon.host}</td><td>{daemon.command}</td><td>{daemon.status}</td></tr>
      }
    }</tbody>
    </table>
  ...
  def convertTasks(status: ServerStatus): Elem =
    <table class={TableTheme.table.htmlClass}>
      <thead class={
      BoincClientStyle.inTextIcon.htmlClass
    }><tr><th>Task</th><th>In progress</th><th>Unsent</th></tr></thead>
      <tbody>{
      status.tasks_by_app.map { tasks =>
        <tr><td>{tasks.name}</td><td>{tasks.in_progress}</td><td>{tasks.unsent}</td></tr>
      }
    }</tbody>
    </table>
  ...
  def convertDBStatus(status: ServerStatus): Elem =
    <table class={TableTheme.table.htmlClass}>
      <tbody>
        <tr><td>{"computing_power".localize}</td><td>{
      status.database_file_states.current_floating_point_speed
    }</td></tr>
        <tr><td>{"hosts_registered_24h".localize}</td><td>{
      status.database_file_states.hosts_registered_in_past_24_hours
    }</td></tr>
        <tr><td>{"hosts_with_credit".localize}</td><td>{status.database_file_states.hosts_with_credit}</td></tr>
        <tr><td>{"hosts_with_recent_credit".localize}</td><td>{
      status.database_file_states.hosts_with_recent_credit
    }</td></tr>
        <tr><td>{"users_registered_24h".localize}</td><td>{
      status.database_file_states.users_registered_in_past_24_hours
    }</td></tr>
        <tr><td>{"users_with_credit".localize}</td><td>{status.database_file_states.users_with_credit}</td></tr>
        <tr><td>{"users_with_recent_credit".localize}</td><td>{
      status.database_file_states.users_with_recent_credit
    }</td></tr>
        ...
        <tr><td>{"results_waiting_for_deletion".localize}</td><td>{
      status.database_file_states.results_waiting_for_deletion
    }</td></tr>
        <tr><td>{"wu_assimilation".localize}</td><td>{
      status.database_file_states.workunits_waiting_for_assimilation
    }</td></tr>
        <tr><td>{"wu_deletion".localize}</td><td>{status.database_file_states.workunits_waiting_for_deletion}</td></tr>
        <tr><td>{"wu_validation".localize}</td><td>{
      status.database_file_states.workunits_waiting_for_validation
    }</td></tr>
  ...
  override def render: Elem =
    <div id="project_overview">
      ...
      <div class={
      FloatingMenu.root.htmlClass
    } style="display:flex;border-left:solid 1px #AAA;border-right:solid 1px #AAA;border-top:solid 1px #AAA;margin-top:-9px">
        {
      <select class={LoginPageStyle.input.htmlClass} style="margin:0" id="project" onchange={jsOnChangeListener}>
            ...
            {
        projects.map(
          _.toList.sortBy(_._2.toLowerCase()).map(project => <option value={project._1}>{project._2}</option>)
        )
      }
          </select>
    }

        <a class={FloatingMenu.active.htmlClass} style="max-width:max-content" href="#loadAll" onclick={
      (_: Event) => loadProjectsFromClients()
    }>
