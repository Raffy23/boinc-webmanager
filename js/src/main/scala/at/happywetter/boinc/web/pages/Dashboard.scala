...
import at.happywetter.boinc.web.css.definitions.Misc as Style
...
object Dashboard extends Layout:
  ...
  override def render: Elem =
    ...
    <div>
      <div class={FloatingMenu.root.htmlClass}>
        <a class={FloatingMenu.active.htmlClass} onclick={
      (event: Event) => {
        event.target
      ...
      sortedClients.map(_.map(c => {
        ...
        <tr>
                  ...
                  <td style="width:240px" class={BoincClientStyle.progressBar.htmlClass}>
                    <progress style="width:calc(100% - 5em);margin-right:20px" value={getDataAttr(_.disk_value)} max={
          getDataAttr(_.disk_max)
        }/>
      ...
      clientsDataSum.currentCPUs.map(c => s"$c / ${clientsDataSum.sumCPUs.now}")
    }</td>
              ...
              <td class={Style.centeredText.htmlClass}>{
      clientsDataSum.runtime.map(r => BoincFormatter.convertTime(r))
    }</td>
              <td class={Style.centeredText.htmlClass}></td>
              <td class={Style.centeredText.htmlClass}></td>
            </tr>
        ...
        <div id="workunits_table_container">
          <table class={
      Seq(TableTheme.table.htmlClass, TableTheme.noBorder.htmlClass).mkString(" ")
    } style="display:none" id="dashboard_workunits_table">
            <thead>
        ...
        clients.map(client => {
          <tr id={s"dashboard_${client._1}_details"}>
            ...
            projects.map(project => {
              val rData =
                client._2.details.map(_.map(_.projects.getOrElse(project._1, List.empty)).getOrElse(List.empty)).now

              val active = rData
                .filter(p => p.activeTask.nonEmpty)
                .count(t =>
                  !t.supsended && Result
                    .ActiveTaskState(t.activeTask.get.activeTaskState) == Result.ActiveTaskState.PROCESS_EXECUTING
                )
              ...
              val time = rData
                ...
                .map(r => r.remainingCPU)
                .sum / (if (active == 0) 1 else active)
            ...
            data._2.details.now
              .map { details =>
                ...
                val timeClient = rData
                  .filter(r => !r.plan.contains("nci"))
                  .map(r => r.remainingCPU)
                  .sum / (if (activeClient == 0) 1 else activeClient)
  ...
  @inline
  private def getDataAttr(f: HostData => String)(implicit
    data: Rx[Option[Either[HostData, Exception]]]
  ): Rx[Option[String]] =
    data.map(_.flatMap(_.swap.map(f).toOption))

  private def getData(f: HostData => String, errorTooltip: Boolean = false, default: String = "-- / --")(implicit
    data: Rx[Option[Either[HostData, Exception]]]
  ): Rx[Node] =
    data.map { dataOption =>
      dataOption
  ...
  private def getNetwork(errorTooltip: Boolean = false, default: String = "-- / --")(implicit
    data: Rx[Option[Either[HostData, Exception]]]
  ): Rx[Node] =
  ...
  private def calculateOffsetOfWorkunitsTable(): Unit =
    ...
    var max = 0d
    dom.document
      .querySelectorAll("#workunits_table_container > table > thead > tr span")
      .forEach((node, _, _) => {
    ...
    dom.document.getElementById("workunits_table_container").asInstanceOf[HTMLElement].style =
      "margin-top:" + max + "px"
