...
import at.happywetter.boinc.web.css.definitions.pages.{BoincClientStyle, BoincMessageStyle as Style}
...
class BoincMessageLayout extends BoincClientLayout:
      ...
      <div id="client-messages" style={selection.map(s => if (s == SELECTION_MESSAGES) None else Some("display:none"))}>
        <div class={Style.filterBox.htmlClass}>
          ...
          <select id="filter_priority" onchange={jsOnMessageFilterChangeAction}>
            ...
            <option value="2" selected={if (filter.now._1 == 2) Some("selected") else None}>{
      "user_alert".localize
    }</option>
            <option value="3" selected={if (filter.now._1 == 3) Some("selected") else None}>{
      "internal_error".localize
    }</option>
            <option value="4" selected={if (filter.now._1 == 4) Some("selected") else None}>{
      "scheduler_alert".localize
    }</option>
          </select>
  ...
  private def convertContent(content: String) =
    ...
    root
      ...
      .forEach((node, _, _) => node.asInstanceOf[HTMLElement].removeAttribute("onclick"))
