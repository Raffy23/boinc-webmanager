...
package object dashboard {
  ...
  case class DetailData(client: String, projects: Map[String, List[Result]] = Map().empty,
                        workunits: List[Workunit] = List(), apps: Map[String, App] = Map().empty)

  case class HostData(cpu: String, memory: String, time: String, deadline: String,
  ...
  case class HostSumData(currentCPUs: Var[Int], sumCPUs: Var[Int], runtime: Var[Double],
