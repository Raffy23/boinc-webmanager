  ...
  class WuTableRow(val result: ReactiveResult)(implicit boinc: BoincClient,ctx: Ctx.Owner) extends DataTable.TableRow {
    override val columns = List(
      ...
      Rx {
        ...
        span(result.uiStatus())
      },
      Rx {
        import scalatags.JsDom.all._

        span(BoincFormater.convertTime(result.pastTime()))
      },
      Rx {
        import scalatags.JsDom.all._

        span(BoincFormater.convertTime(result.remainingCPU()))
      },
      Rx {
        import scalatags.JsDom.all._

        span(BoincFormater.convertDate(new Date(result.reportDeadline()*1000)))
      },
      Rx {
        import scalatags.JsDom.all._

        span(result.app().map(_.userFriendlyName).getOrElse(result.wuName.now).asInstanceOf[String])
      },
      Rx {
        import scalacss.ScalatagsCss._
        import scalatags.JsDom.all._
