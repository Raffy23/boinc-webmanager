...
trait DatabaseProvider:
  ...
  private val MODE_RW = IDBTransactionMode.readwrite

  protected def transactionAsync[A](
    f: IDBObjectStore => Future[A]
  )(implicit storeNames: js.Array[String], objStore: String): Future[A] =
  ...
  private def firefoxTransaction[A](
    firefoxCallback: IDBObjectStore => A
  )(implicit storeNames: js.Array[String], objStore: String): Future[A] =
    database.map(r => {
  ...
  private def firefoxTransactionAsync[A](
    firefoxCallback: IDBObjectStore => Future[A]
  )(implicit storeNames: js.Array[String], objStore: String): Future[A] =
    database.flatMap(r => {
      val ffTransaction = r.transaction(storeNames, MODE_RW)
      val objStorage = ffTransaction.objectStore(objStore)
  ...
  implicit protected def IDBRequestToFuture[A](request: IDBRequest[_, _]): Future[Option[A]] =
    new Promise[Option[A]]((resolve, reject) => {
      request.onsuccess = _ => resolve(request.result.asInstanceOf[js.UndefOr[A]].toOption)
      request.onerror = reject
    }).toFuture

  implicit class IDBRequestStringFuture(request: IDBRequest[_, _]):
