...
import at.happywetter.boinc.shared.boincrpc.HostInfo
    ...
    <div id="host-info">
      ...
      <div id="boinc_cc_state">
        ...
        <table class={s"${TableTheme.table.htmlClass} ${TableTheme.noBorder.htmlClass}"} style="width:auto!important">
          ...
          <tbody>
            <tr>
              <td> <i class="fa fa-tasks" aria-hidden="true" style="display:inline-block;width:1em;padding-right:1em"></i> {
      "boinc_info_run_mode".localize
    } </td>
              <td> <input type="radio" name="run_mode" value="always" checked={clientCC.map(_.taskMode == 1)} onclick={
      action(boinc.setRun, Modes.Always)
    }></input> </td>
              <td> <input type="radio" name="run_mode" value="auto"   checked={clientCC.map(_.taskMode == 2)} onclick={
      action(boinc.setRun, Modes.Auto)
    }></input> </td>
              <td> <input type="radio" name="run_mode" value="never"  checked={clientCC.map(_.taskMode == 3)} onclick={
      action(boinc.setRun, Modes.Never)
    }></input> </td>
            </tr>
            <tr>
              <td> <i class="fa fa-tv" aria-hidden="true" style="display:inline-block;width:1em;padding-right:1em"></i> {
      "boinc_info_gpu_mode".localize
    } </td>
              <td> <input type="radio" name="gpu_mode" value="always" checked={clientCC.map(_.gpuMode == 1)} onclick={
      action(boinc.setGpu, Modes.Always)
    }></input> </td>
              <td> <input type="radio" name="gpu_mode" value="auto"   checked={clientCC.map(_.gpuMode == 2)} onclick={
      action(boinc.setGpu, Modes.Auto)
    }></input> </td>
              <td> <input type="radio" name="gpu_mode" value="never"  checked={clientCC.map(_.gpuMode == 3)} onclick={
      action(boinc.setGpu, Modes.Never)
    }></input> </td>
            </tr>
            <tr>
              <td> <i class="fa fa-exchange-alt" aria-hidden="true" style="display:inline-block;width:1em;padding-right:1em"></i> {
      "boinc_info_network_mode".localize
    } </td>
              <td> <input type="radio" name="network_mode" value="always" checked={
      clientCC.map(_.networkMode == 1)
    } onclick={action(boinc.setNetwork, Modes.Always)}></input> </td>
              <td> <input type="radio" name="network_mode" value="auto"   checked={
      clientCC.map(_.networkMode == 2)
    } onclick={action(boinc.setNetwork, Modes.Auto)}></input> </td>
              <td> <input type="radio" name="network_mode" value="never"  checked={
      clientCC.map(_.networkMode == 3)
    } onclick={action(boinc.setNetwork, Modes.Never)}></input> </td>
            </tr>
          </tbody>
        </table>
        <table class={s"${TableTheme.table.htmlClass} ${Style.table.htmlClass}"} style="line-height:1.4567">
          <tbody>
            <tr><td style="width:55px"><b>{"boinc_info_version".localize}</b></td><td>{
      clientData.map(_.boincVersion)
    }</td></tr>
            ...
            <tr><td><b>{"boinc_info_disk".localize}</b></td> <td>
              ...
              <progress style="width:250px" value={progressBarValue} max={
      clientData.map(_.hostInfo.diskTotal.toString)
    }></progress>
              </span>
              <br/>
              {
