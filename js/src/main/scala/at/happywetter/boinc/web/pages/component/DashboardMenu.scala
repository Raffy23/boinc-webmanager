...
object DashboardMenu:
  ...
  case class TopLevelEntry(override val name: String, override val href: String, reference: Option[String] = None)
      extends MenuEntry(name, href)
  case class SublistEntry(override val name: String,
                          override val href: String,
                          visible: Var[Boolean] = Var(false),
                          subMenuRef: String,
                          reference: Option[String] = None,
                          subMenu: Var[List[SubMenuEntry]] = Var(List.empty)
  ) extends MenuEntry(name, href)
  case class SubMenuEntry(override val name: String, override val href: String, reference: Option[String] = None)
      extends MenuEntry(name, href)
      ...
      menuComputers.map(_.sortBy(_.name).map {
        ...
        case entry: SublistEntry =>
          <li class={Seq(Style.elem.htmlClass, Style.clickable.htmlClass).mkString(" ")}>
              ...
              <ul data-submenu-id={entry.subMenuRef} style={
            entry.visible.map(v => s"display:${if (v) "block" else "none"}")
          }>
                {
            entry.subMenu.map(
              _.map(entry =>
                <li class={Style.elem.htmlClass}>
                  <a href={entry.href} data-menu-id={entry.reference} data-navigo={true} onclick={
                  selectionListener
                } style="width:unset">
                    {entry.name}
                  </a>
  ...
  def addComputerToGroup(linkUrl: String,
                         elementName: String,
                         submenu: String,
                         reference: Option[String] = None
  ): Unit =
    def find(menu: List[MenuEntry]): SublistEntry =
      menu
        .find(x => x.isInstanceOf[SublistEntry] && x.asInstanceOf[SublistEntry].subMenuRef == submenu)
        .get
        .asInstanceOf[SublistEntry]
  ...
  def selectMenuItemByContent(content: String): Unit =
    import at.happywetter.boinc.web.facade.NodeListConverter.convNodeList
    dom.document
      .querySelectorAll("#dashboard-menu li a")
      .forEach((a, _, _) => {
